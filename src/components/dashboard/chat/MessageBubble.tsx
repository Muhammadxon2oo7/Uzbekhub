...
import { motion } from "framer-motion";
import Image from "next/image";
import { Play, Pause, Check, CheckCheck, Smile, Reply, Forward } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
...
export default function MessageBubble({ message, addReaction, onReply }: MessageBubbleProps) {
  return (
    <div className={`flex ${message.isOwn ? "justify-end" : "justify-start"}`}>
      <div className={`max-w-xs lg:max-w-md group ${message.isOwn ? "order-2" : "order-1"}`}>
        <div
        ...
        >
          {message.replyTo && (
            <div className="mb-2 p-2 bg-white/5 rounded-lg border-l-4 border-primary/50">
              <p className="text-xs text-gray-400">{message.replyTo.sender}</p>
        ...
        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 mt-1">
          ...
          <Button size="icon" variant="ghost" className="w-6 h-6 hover:bg-white/10" onClick={onReply}>
            <Reply className="w-3 h-3" />
          </Button>
          <Button size="icon" variant="ghost" className="w-6 h-6 hover:bg-white/10">
            <Forward className="w-3 h-3" />
