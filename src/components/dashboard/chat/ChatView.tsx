...
import MessageInput from "./MessageInput";
import MessageBubble, { Message } from "./MessageBubble";
import { getUserById, uploadFile } from "@/lib/api";
...
export default function ChatView() {
  ...
  useEffect(() => {
    ...
    if (selectedChat) {
      ...
      if (currentChat?.isGroup) {
        ws = new WebSocket(`wss://${process.env.NEXT_PUBLIC_WEBSOKKED}/ws/chat/group/${currentChat.name}/`, ["Bearer", token]);
      } else {
        ws = new WebSocket(`wss://${process.env.NEXT_PUBLIC_WEBSOKKED}/ws/chat/users/`, ["Bearer", token]);
      }
      ...
      ws.onmessage = (event) => {
        ...
        if (Array.isArray(data)) {
          // Guruh xabarlari massiv sifatida keladi
          data.forEach((msg: any) => {
            newMessage = {
              id: msg.message_id || msg.id,
              sender: msg.sender.username,
              content: msg.message,
              time: new Date(msg.created_at).toLocaleTimeString("uz-UZ", { hour: "2-digit", minute: "2-digit" }),
              isOwn: msg.sender.id.toString() === localStorage.getItem("userId"),
              type: msg.file_url ? (msg.file_url.match(/\.(mp3|wav|ogg)$/) ? "voice" : "image") : "text",
              status: msg.is_delivery ? "delivered" : "sent",
              replyTo: msg.reply_to
                ? {
                    id: msg.reply_to.id,
                    content: msg.reply_to.message,
          ...
          newMessage = {
            ...
            time: new Date(data.created_at).toLocaleTimeString("uz-UZ", { hour: "2-digit", minute: "2-digit" }),
            isOwn: data.sender.id.toString() === localStorage.getItem("userId"),
            type: data.file_url ? (data.file_url.match(/\.(mp3|wav|ogg)$/) ? "voice" : "image") : "text",
            status: data.is_delivery ? "delivered" : "sent",
            replyTo: data.reply_to
              ? {
                  id: data.reply_to.id,
                  content: data.reply_to.message,
                  sender: data.reply_to.sender.username,
                }
              : undefined,
          };
        ...
        const updatedChats = chats.map((chat) =>
          chat.id === selectedChat
            ? { ...chat, lastMessage: newMessage.content, time: newMessage.time, unread: chat.unread + (newMessage.isOwn ? 0 : 1) }
            : chat
        );
        saveChatsToStorage(updatedChats);
      };
  ...
  const handleSendMessage = () => {
    if (!message.trim() || !websocket || !selectedChat) return;

    const chats = getChatsFromStorage();
    const currentChat = chats.find((chat) => chat.id === selectedChat);
    const messageData = {
      message,
      user_id: currentChat?.isGroup ? undefined : selectedChat,
      file_url: "",
      reply_to: replyTo?.id,
    };
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!selectedChat || !websocket || !e.target.files || e.target.files.length === 0) return;
    ...
    try {
      ...
      const fileType: "image" | "voice" = file.type.startsWith("image/") ? "image" : file.type.startsWith("audio/") ? "voice" : "image";
  ...
  const addReaction = (messageId: string, emoji: string) => {
    console.log("Reaktsiya:", messageId, emoji);
    // Backendda reaction funksiyasi yo'q, agar qo'shmoqchi bo'lsangiz, API endpoint qo'shish kerak
  };
                ...
                <AnimatePresence>
                  {messagesByChatId[selectedChat]?.map((msg, index) => (
                    <motion.div
                      ...
                      transition={{ delay: index * 0.05, duration: 0.3 }}
                    >
                      <MessageBubble message={msg} addReaction={addReaction} onReply={() => handleReply(msg)} />
                    </motion.div>
                  ))}
                </AnimatePresence>
                <div ref={messagesEndRef} />
